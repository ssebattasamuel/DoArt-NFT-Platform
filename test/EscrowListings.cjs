...
describe('EscrowListings Contract', function () {
  let DoArt, EscrowStorage, EscrowAuctions, EscrowListings;
  let doArt, escrowStorage, escrowAuctions, escrowListings;
  ...
  describe('Role Initialization', function () {
    it('Should set correct roles in constructor', async function () {
      ...
      // Verify EscrowListings has ADMIN_ROLE in EscrowStorage
  ...
  describe('Update Listing', function () {
    ...
    it('Should allow seller to update listing price', async function () {
      ...
      await expect(
        escrowListings.connect(seller).updateListing(doArt.address, 1, newPrice)
      )
        .to.emit(escrowListings, 'ListingUpdated')
        .withArgs(doArt.address, 1, seller.address, newPrice);
    ...
    it('Should revert if non-seller tries to update', async function () {
      await expect(
        escrowListings.connect(other).updateListing(doArt.address, 1, price)
    ...
    it('Should revert if listing is an auction', async function () {
      ...
      await expect(
        escrowListings.connect(seller).updateListing(doArt.address, 1, price)
