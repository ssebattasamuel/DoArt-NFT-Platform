  ...
  const onSubmit = async (data) => {
    if (isMint) {
      try {
        ...
        if (invalidItems) {
          toast.error(
            'Cannot submit: Some items have invalid or zero ETH equivalent prices. Please check conversions.',
            { id: 'submit-error' }
          );
          return;
        }
        if (
          data.items.some(
            (item) => !item.purchasePrice || Number(item.purchasePrice) <= 0
          )
        ) {
          toast.error('All items must have a valid positive price', {
            id: 'price-error'
          });
          return;
        }
        ...
        contracts?.escrowListings && (
          <StyledBatchForm onSubmit={handleSubmit(onSubmit)} type="modal">
            ...
            <FormFooter>
              ...
              <p>Items: {fields.length}/50</p>
              <Button
                type="submit"
                disabled={isMinting || isConverting || isSubmitting}
              >
                {isMinting || isSubmitting ? <Spinner /> : 'Submit'}
              </Button>
              <Button
                type="button"
                variation="secondary"
