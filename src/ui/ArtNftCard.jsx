...
function ArtNftCard({ nft }) {
  console.log('ArtNftCard rendered', nft.tokenId);
  const {
    contractAddress,
    tokenId,
    listing,
    ...
    <Card>
      ...
      <Info>
        ...
        {listing?.isListed && !isAuctionActive ? (
          <>
            <Price>
              <Tooltip text="Price in original currency, current USD and ETH, based on market rates.">
                {metadata?.originalCurrency === 'USD'
                  ? `$${metadata.originalPrice} USD ($${usdPrice} current) (${formatCurrency(ethers.utils.formatEther(listing.price), 'ETH')})`
                  : metadata?.originalCurrency === 'ETH'
                    ? `${formatCurrency(metadata.originalPrice, 'ETH')} ($${usdPrice})`
